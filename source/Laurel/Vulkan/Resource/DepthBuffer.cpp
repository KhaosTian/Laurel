...
DepthBuffer::DepthBuffer(CommandPool& cmd_pool, VkExtent2D extent):
    ...
    // 指定用途为深度/模板附件
    m_image(std::make_unique<Image>(cmd_pool.device(),
                                    extent,
                                    1, // Mip层级
                                    m_format,
    ...
    m_image_memory(std::make_unique<DeviceMemory>(m_image->AllocateMemory(VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT // 优先使用GPU本地内存
                                                                          ))),

    // 创建深度图像的视图
    // 指定只关注深度部分，不含模板，即使格式可能包含模板
    m_image_view(std::make_unique<ImageView>(cmd_pool.device(),
    ...
    // 确保图像在渲染管线中正确用作深度附件
    // 此操作通过命令缓冲区提交物理设备操作
    m_image->TransitionImageLayout(cmd_pool,
...
VkFormat DepthBuffer::FindDepthFormat(const Device& device) {
    return FindSupportFormat(device,
                             // 候选格式优先级（按设备支持广泛性排序）：
                             {
...
VkFormat DepthBuffer::FindSupportFormat(const Device&                device,
