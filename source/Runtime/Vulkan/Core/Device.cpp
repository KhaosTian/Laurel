...
#include "Runtime/Vulkan/Core/Enumerate.h"
...
Device::Device(VkPhysicalDevice physical_device, const Surface& surface, const std::vector<const char*>& required_extensions, const VkPhysicalDeviceFeatures& device_features):
    ...
    const std::set     queue_family_indices = { m_queue_indices.graphics, m_queue_indices.present, m_queue_indices.compute };
...
void Device::CheckRequiredExtensionsSupport(const std::vector<const char*>& required_extensions) const {
    const auto available_extensions = GetEnumerateVector(m_physical_device, static_cast<const char*>(nullptr), vkEnumerateDeviceExtensionProperties);
